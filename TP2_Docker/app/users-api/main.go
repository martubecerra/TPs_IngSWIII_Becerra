...
func main() {
   ...
   env := os.Getenv("ENV")
   if env == "QA" {
	   log.Println("[QA] Configuración de entorno de pruebas")
   } else if env == "PROD" {
	   log.Println("[PROD] Configuración de entorno de producción")
   } else {
	   log.Println("[DEFAULT] Configuración por defecto")
   }
